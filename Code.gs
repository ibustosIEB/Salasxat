...
function setupSheets() {
  ...
  // Guardar l'ID d'aquest Spreadsheet per si el doGet el necessita
...
// ── SPREADSHEET ACTIU ───────────────────────────────────────────
function getSS() {
  // En context de doGet no hi ha Spreadsheet actiu: usem l'ID guardat
  var id = PropertiesService.getScriptProperties().getProperty(SS_ID_KEY);
  if (id) return SpreadsheetApp.openById(id);
  // Fallback: si s'executa des de l'editor, usem l'actiu i guardem l'ID
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  PropertiesService.getScriptProperties().setProperty(SS_ID_KEY, ss.getId());
  return ss;
}
